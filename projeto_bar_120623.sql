...
CREATE DATABASE IF NOT EXISTS `projeto_bar` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci */;
USE `projeto_bar`;
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='Registra as principais alterações nesse BD';

-- Copiando dados para a tabela projeto_bar.auditoria: ~1 rows (aproximadamente)
INSERT INTO `auditoria` (`codAuditoria`, `acao`, `tabela`, `dataHora`, `usuario`) VALUES
	(4, 'Cliente inserido: Esau com CPF:  e endereço: ', 'cliente', '2023-06-12 20:56:44', 'root@');
...
-- Copiando dados para a tabela projeto_bar.categoria: ~1 rows (aproximadamente)
...
-- Copiando dados para a tabela projeto_bar.cliente: ~0 rows (aproximadamente)
INSERT INTO `cliente` (`codCliente`, `nomeCliente`, `enderecoCliente`, `telefoneCliente`, `cpfCliente`, `dataNascimento`) VALUES
	(14, 'Esau', 'Rua Antonio Correa Filho', '3599999999', '111111111', '2003-10-01 00:00:00');
...
-- Copiando estrutura para tabela projeto_bar.funcionario
...
-- Copiando dados para a tabela projeto_bar.funcionario: ~0 rows (aproximadamente)
INSERT INTO `funcionario` (`codFuncionario`, `nome`, `salario`, `dataNascimento`, `cpf`) VALUES
	(1, 'Pedro Henrique', 1000, '2002-03-01 00:00:00', '9999999');
...
-- Copiando dados para a tabela projeto_bar.marca: ~2 rows (aproximadamente)
...
-- Copiando estrutura para procedure projeto_bar.relatorioVendas
DELIMITER //
CREATE PROCEDURE `relatorioVendas`()
BEGIN
SELECT COUNT(venda) AS v
WHERE v.CLIENTE_codCliente;
if (@contador = 0)
	then SELECT "Cliente ainda não comprou!" AS mensagem;
	ELSE 
			SELECT v.codVenda, date_format(v.dataHora,"%d de %M de %Y %H:%i:%s") AS dataHora, 
			c.nome FROM venda AS v INNER JOIN cliente AS c
			ON v.CLIENTE_codCliente = c.codCliente 
			WHERE v.CLIENTE_codCliente ORDER BY v.codVenda;
END if;
END//
DELIMITER ;
...
-- Copiando dados para a tabela projeto_bar.venda: ~0 rows (aproximadamente)

-- Copiando estrutura para view projeto_bar.visualiza_funcionario
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `visualiza_funcionario` (
...
-- Copiando estrutura para view projeto_bar.visualiza_idade
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `visualiza_idade` (
	`Nome do Cliente` VARCHAR(200) NOT NULL COLLATE 'utf8_general_ci',
	`Data de Nascimento` VARCHAR(10) NULL COLLATE 'utf8mb4_general_ci'
) ENGINE=MyISAM;

-- Copiando estrutura para view projeto_bar.visualiza_produto
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `visualiza_produto` (
	`Código do Produto` INT(11) NOT NULL,
	`Nome do Produto` VARCHAR(200) NOT NULL COLLATE 'utf8_general_ci',
	`Preço de Custo` DECIMAL(10,2) UNSIGNED NOT NULL,
	`Preco de Venda` DECIMAL(10,2) NOT NULL
) ENGINE=MyISAM;
...
DROP TABLE IF EXISTS `visualiza_funcionario`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `visualiza_funcionario` AS SELECT nome AS Nome , salario AS Salário, cpf AS CPF, DATE_FORMAT(dataNascimento,"%d/%m/%Y") AS "Data de Nascimento" FROM funcionario ;

-- Copiando estrutura para view projeto_bar.visualiza_idade
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `visualiza_idade`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `visualiza_idade` AS SELECT nomeCliente AS "Nome do Cliente", DATE_FORMAT(dataNascimento,"%d/%m/%Y") AS "Data de Nascimento" FROM cliente ;

-- Copiando estrutura para view projeto_bar.visualiza_produto
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `visualiza_produto`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `visualiza_produto` AS SELECT codProduto AS "Código do Produto", nomeProduto AS "Nome do Produto", precoCusto AS "Preço de Custo", precoVenda AS "Preco de Venda" FROM produto ;
